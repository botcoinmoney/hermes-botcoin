"""Reasoning-trace v3 normalization.

The coordinator validates traces strictly: string `step_id`s, no `task`/`step`
fields, `compute_logic.inputs` references prior step_ids by string. These
tests exercise the legacy-input migration paths because the agent's solver
output drifts depending on the model.
"""
...
from hermes_botcoin.trace import (  # noqa: E402
    SUPPORTED_OPS,
    normalize_trace,
...
def test_revision_step_passthrough():
    raw = [{"step_id": "rev1", "action": "revision",
            "note": "Previous attempt passed 5/8 — re-examining acrostic."}]
    out = normalize_trace(raw)
