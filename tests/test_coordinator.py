...
from hermes_botcoin.coordinator import (  # noqa: E402
    Coordinator,
    CoordinatorError,
    backoff_seconds,
    is_retryable,
...
def test_is_retryable_matrix():
    ...
    # Any 401 is retryable — caller decides whether to refresh the bearer.
    assert is_retryable(CoordinatorError(status=401, route="/x", error="token_expired"))
    assert is_retryable(CoordinatorError(status=401, route="/x", error="Authentication required"))
